...
public:
  ...
  virtual void propagate_flow_group(int ith) {}
  virtual void commit_flow_group(int ith) {}
  virtual void init_flow_group(int ith) {}
};
...
//      return (*ptr_v.get())[x][y][i];
      return (*ptr_v.get())[x][y][((dy&1)<<1)|(((dx&1)&((dx&2)>>1))|((dy&1)&((dy&2)>>1)))];
    }
  };
...
static milliseconds elapsed = milliseconds(0);


void* Init_flow(void *args) {
  int ith = *((int*) args);

  mf->init_flow_group(ith);

  return 0;
}

void* Propagate_flow(void *args) {
  int ith = *((int*) args);

  mf->propagate_flow_group(ith);

  return 0;
}

void* Commit_flow(void *args) {
  int ith = *((int*) args);

  mf->commit_flow_group(ith);
...
//      return v[x][y][i];
  ...
  std::vector<VectorField<Tf>> velocity_flow_par;
  std::vector<std::vector<std::vector<int>>> last_use_par;
  std::vector<pthread_t> threads;
  std::vector<bool> props;

  std::mutex lu_mute;
  std::mutex v_flow_mute;

  int UT = 0;
  static constexpr size_t T = 1'0;
  double g = 0.1;
  size_t N, M;
  size_t iter = 0;
...
public:
  ...
  void init(const InitArgs &initArgs) override {
    ...
    velocity_flow_par = std::vector<VectorField<Tf>>(initArgs.parN, velocity_flow);
    last_use_par = std::vector<std::vector<std::vector<int>>>(initArgs.parN, last_use);

    threads = std::vector<pthread_t>(initArgs.parN);
    props = std::vector<bool>(initArgs.parN);
  ...
  void init_flow_group(int ith) override {
    last_use_par[ith] = last_use;
    velocity_flow_par[ith] = velocity_flow;
  }
  void propagate_flow_group(int ith) override {
//    size_t parN = last_use_par.size();
//
//    size_t xBeg = parN==1 ? 0 : (size_t)((N * ith) / parN);
//    size_t xEnd = parN==1 ? N : (size_t)((N * (ith+1)) / parN);
//
//    props[ith] = 0;
//    for (size_t x = xBeg; x < xEnd; ++x) {
//      for (size_t y = 0; y < M; ++y) {
//        if (field[x][y] != '#' && last_use_par[ith][x][y] != UT) {
//          auto [t, local_prop, _] = propagate_flow(x, y, 1, ith);
//          if (t > static_cast<Tv>(0)) {
//            props[ith] = 1;
//          }
//        }
...
//    }
    ...
    int yBeg = parN==1 ? 0 : (int)((M * ith) / parN);
    int yEnd = parN==1 ? M : (int)((M * (ith+1)) / parN);


    props[ith] = 0;
//    for (int x = 0; x < N; ++x) {
//      for (int y = ith; y < M; y+=parN) {
//        if (field[x][y] != '#' && last_use_par[ith][x][y] != UT) {
//          auto [t, local_prop, _] = propagate_flow(x, y, 1, ith);
//          if (t > static_cast<Tv>(0)) {
//            props[ith] = 1;
...
//    }
  ...
  void commit_flow_group(int ith) override {
    size_t parN = last_use_par.size();
    ...
    for (size_t x = xBeg; x < xEnd; ++x) {
      for (size_t y = 0; y < M; ++y) {
        if (field[x][y] != '#') {
          for (auto ith = 0; ith < last_use_par.size(); ++ith) {
            if (velocity_flow.v[x][y] != velocity_flow_par[ith].v[x][y]) {
              last_use[x][y] = UT;
              velocity_flow.v[x][y] = velocity_flow_par[ith].v[x][y];
              break;
            }
          }
        }
  ...
  void run() override {
    ...
    std::vector<int> indexes(last_use_par.size());
...
//        for(int ith=0; ith < last_use_par.size(); ith++) {
//          last_use_par[ith] = last_use;
//          velocity_flow_par[ith] = velocity_flow;
//          props[ith] = 0;
//        }
        ...
        for(int ith=0; ith < last_use_par.size(); ith++)
          pthread_create(&(threads[ith]), 0, Init_flow, &(indexes[ith]));// запуск потоков

        for (auto ith=0; ith<last_use_par.size(); ith++)                                                              // ожидание окончания
          pthread_join(threads[ith], 0);


        for(int ith=0; ith < last_use_par.size(); ith++)
          pthread_create(&(threads[ith]), 0, Propagate_flow, &(indexes[ith]));// запуск потоков
        ...
        if (prop) {
          for(int ith=0; ith < last_use_par.size(); ith++)
            pthread_create(&(threads[ith]), 0, Commit_flow, &(indexes[ith]));// запуск потоков
...
//        }
      ...
      std::cout << "Elapsed time " << iter << ": "<< duration_cast<milliseconds>(end - start).count() << " ms\n";
...
//      }
  ...
  tuple<Tv, bool, pair<int, int>> propagate_flow(int x, int y, Tv lim, int p) {
    last_use_par[p][x][y] = UT - 1;
    Tv ret = 0;
    for (auto [dx, dy] : deltas) {
      int nx = x + dx, ny = y + dy;
      bool flag = true;
      for (int ith = 0; ith < last_use_par.size() && flag; ++ith) {
        flag = flag && (last_use_par[ith][nx][ny] < UT);
      }

      if (field[nx][ny] != '#' && flag) {
        auto cap = velocity.get(x, y, dx, dy);
        auto flow = velocity_flow_par[p].get(x, y, dx, dy);
        if (flow == static_cast<Tf>(cap)) {
          continue;
        }
        // assert(v >= velocity_flow.get(x, y, dx, dy));
        auto vp = min(lim, cap - static_cast<Tv>(flow));
        if (last_use_par[p][nx][ny] == UT - 1) {
          velocity_flow_par[p].add(x, y, dx, dy, vp);
          last_use_par[p][x][y] = UT;
          // cerr << x << " " << y << " -> " << nx << " " << ny << " " << vp << " / " << lim << "\n";
          return {vp, 1, {nx, ny}};
        }
        auto [t, prop, end] = propagate_flow(nx, ny, vp, p);
        ret += t;
        if (prop) {
          velocity_flow_par[p].add(x, y, dx, dy, t);
          last_use_par[p][x][y] = UT;
          // cerr << x << " " << y << " -> " << nx << " " << ny << " " << t << " / " << lim << "\n";
          return {t, prop && end != pair(x, y), end};
        }
...
int main(int argc, char** argv) {
  ...
  for(int i=1; i<argc; ++i) {
    ...
    if(arg.rfind("--p-type=", 0) == 0) {
     ...
     sscanf(arg.data(), "--parallel=%ul", &parN);
     std::cout << "Parallel = " << parN << std::endl;
   }
  }
  ...
  auto elapsed_all = duration_cast<milliseconds>(end - start);
  std::cout << "Elapsed time: " << elapsed_all.count() << " ms\n";
  std::cout << "Elapsed time prop_flow: " << elapsed.count() << " ms\n";
